...
export default function BookingSummary({ service, barber, date, time }) {
  ...
  return (
    <div className="rounded-3xl border border-beige/70 bg-white/55 p-5 shadow-xl shadow-sage-deep/10 backdrop-blur-md">
      ...
      <ul className="flex flex-col gap-2.5 text-sm">
        {service && (
          <>
            <li className="flex items-center justify-between gap-2 text-ink/80">
              <span className="flex items-center gap-2">
                <service.icon size={14} className="text-sage-deep" aria-hidden="true" />
                {service.name}
              </span>
            ...
            <li className="flex items-center justify-between gap-2 text-ink/80">
              <span className="flex items-center gap-2">
                <Wallet size={14} className="text-sage-deep" aria-hidden="true" />
                Seña a pagar
              </span>
        ...
        {barber && (
          <li className="flex items-center gap-2 text-ink/80">
            <User size={14} className="text-sage-deep" aria-hidden="true" />
            {barber.name}
          </li>
        )}
        {date && (
          <li className="flex items-center gap-2 capitalize text-ink/80">
            <CalendarDays size={14} className="text-sage-deep" aria-hidden="true" />
            {formatDateLong(date)}
          </li>
        )}
        {time && (
          <li className="flex items-center gap-2 text-ink/80">
            <Clock size={14} className="text-sage-deep" aria-hidden="true" />
